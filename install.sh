...
rebuild_links_from_arrays() {
    ...
    for i in "${!inbound_tags[@]}"; do
        ...
        # 添加到特定协议
        case "$protocol" in
            "Reality") reality_links_content+="${line}\n" ;;
            "Hysteria2") hysteria2_links_content+="${line}\n" ;;
            "SOCKS5") socks5_links_content+="${line}\n" ;;
            "ShadowTLS") shadowtls_links_content+="${line}\n" ;;
            "HTTPS") https_links_content+="${line}\n" ;;
            "AnyTLS") anytls_links_content+="${line}\n" ;;
        esac
    done
}
