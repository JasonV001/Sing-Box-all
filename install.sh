...
parse_ss_link() {
  ...
  \"password\": \"${password}\"
        ...
        OUTBOUND_TAG="relay"
        print_success "Shadowsocks 中转配置解析完成"
    else
    ...
    while true; do
        ...
        case $r_choice in
            1)
                ...
                echo -e "  ${GREEN}SOCKS5:${NC} socks5://[user:pass@]server:port"
                echo -e "  ${GREEN}HTTP/HTTPS:${NC} http(s)://[user:pass@]server:port"
                echo -e "  ${GREEN}Shadowsocks:${NC} ss://base64编码"
                echo -e "  ${GREEN}VMess:${NC} vmess://base64编码"
                echo -e "  ${GREEN}VLESS:${NC} vless://uuid@server:port?参数"
                echo -e "  ${GREEN}Trojan:${NC} trojan://password@server:port?参数"
                echo ""
