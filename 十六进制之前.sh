    ...
    for ((i=0; i<inbounds_count; i++)); do
        ...
        case "$type" in
            ...
            "hysteria2")
                ...
                if [[ -n "$password" ]]; then
                    # URL 编码密码
                    local encoded_password=$(echo -n "${password}" | python3 -c "import sys; from urllib.parse import quote; print(quote(sys.stdin.read().strip()))" 2>/dev/null || echo "${password}")
                    local link="hysteria2://${encoded_password}@${SERVER_IP}:${port}?insecure=1&sni=${sni}#Hysteria2-${SERVER_IP}"
...
setup_hysteria2() {
  ...
  \"tls\": {
    ...
    # URL 编码密码（处理 base64 中的特殊字符）
    local encoded_password=$(echo -n "${HY2_PASSWORD}" | python3 -c "import sys; from urllib.parse import quote; print(quote(sys.stdin.read().strip()))" 2>/dev/null || echo "${HY2_PASSWORD}")
    
    LINK="hysteria2://${encoded_password}@${SERVER_IP}:${PORT}?insecure=1&sni=${HY2_SNI}#Hysteria2-${SERVER_IP}"
    PROTO="Hysteria2"
